...
# Translate predicted gesture to Hindi (using Google Translate)
def translate_to_hindi(gesture):
    translation_dict = {
        'gesture1': 'अंगूठा',  # Example mapping, replace with actual gestures
        'gesture2': 'हाथ',
        # Add all necessary gesture translations
    }

    # If gesture is found in dictionary, return the translation
    if gesture in translation_dict:
        return translation_dict[gesture]

    # If not, use Google Translate to translate the gesture to Hindi
    translator = Translator()
    translated = translator.translate(gesture, src='en', dest='hi')
...
def inference_thread(frame_queue, model, labels):
    ...
    engine.setProperty('voice', voices[1].id)  # You can try voices[0] for male, voices[1] for female
    engine.setProperty('rate', 150)  # Adjust rate of speech
    ...
    while True:
        if not frame_queue.empty():
            ...
            # Speak the predicted gesture in English
            print(f"Speaking: {predicted_label}")
            engine.say(predicted_label)
            engine.runAndWait()

            # Translate the predicted gesture to Hindi (without speaking it)
            translated_label = translate_to_hindi(predicted_label)
            print(f"Translated Gesture in Hindi: {translated_label}")
